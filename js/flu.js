...
// Function to start the confetti and show the overlay message
document.querySelector('.heart').addEventListener('click', function() {
    // Show the overlay with the Yayyy!! message
...
// Function to create heart-shaped confetti
function createConfetti() {
    const confetti = document.createElement('div');
    confetti.classList.add('confetti');
    confetti.innerHTML = '❤️';
    
    // Randomize the confetti's position
    confetti.style.left = `${Math.random() * 100}%`;
    confetti.style.animationDuration = `${Math.random() * 2 + 3}s`; // Random duration for falling
